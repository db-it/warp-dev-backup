...
def test_should_invoke_callback_with_params():
    context = AppContext()
    ...
    with mock.patch('os.walk') as mock_walk:
        ...
        scan_tree(context, config, start_path, mock_callback)
...
def test_should_invoke_callback_on_sentinel_match_by_regex(sentinel_files):
    context = AppContext()
    config = MagicMock(spec=Config)
    config.exclusion_path_sentinels = [
...
def test_should_not_invoke_callback_on_non_existent_sentinel_or_dir():
    context = AppContext()
    config = MagicMock(spec=Config)
    config.exclusion_path_sentinels = [
        {'sentinel': r'pyproject.toml|setup.[py|cfg]', 'dir': 'venv'},
